    ...
    execute: function () {
      ...
      _export("Match3GridComponent", Match3GridComponent = (_dec = ccclass('Match3GridComponent'), _dec2 = property({
        ...
        renderGrid(grid) {
          ...
          for (let r = 0; r < this.rows; r++) {
            for (let c = 0; c < this.cols; c++) {
              var _grid$r, _this$cellNodes$r$c, _this$cellNodes$r;

              const cell = (_grid$r = grid[r]) == null ? void 0 : _grid$r[c];
              if (!cell) continue;
              let node = (_this$cellNodes$r$c = (_this$cellNodes$r = this.cellNodes[r]) == null ? void 0 : _this$cellNodes$r[c]) != null ? _this$cellNodes$r$c : null; // 首次创建节点
              ...
              if (node) {
                ...
                node.setPosition(new Vec3(originX + c * this.cellSize, originY - r * this.cellSize, 0)); // 更新棋子精灵显示
        ...
        ensureCellComponent(node) {
          ...
          button.interactable = true;
          button.target = node;
          let cellComp = node.getComponent(_crd && CellComponent === void 0 ? (_reportPossibleCrUseOfCellComponent({
            error: Error()
          }), CellComponent) : CellComponent);
        ...
        updateCellSprite(node, cell) {
          const sprite = node.getComponent(Sprite); // 根据棋子类型动态加载对应精灵帧
        ...
        selectCell(row, col) {
          ...
          const node = (_this$cellNodes$row = this.cellNodes[row]) == null ? void 0 : _this$cellNodes$row[col];
        ...
        deselectCell() {
          ...
          if (this.selectedRow >= 0 && this.selectedCol >= 0) {
            ...
            const node = (_this$cellNodes$this$ = this.cellNodes[this.selectedRow]) == null ? void 0 : _this$cellNodes$this$[this.selectedCol];
        ...
        trySwap(r1, c1, r2, c2) {
          ...
          const result = this.engine.swap(grid, r1, c1, r2, c2); // 取消原选中状态
        ...
        shakeCell(row, col) {
          ...
          const node = (_this$cellNodes$row2 = this.cellNodes[row]) == null ? void 0 : _this$cellNodes$row2[col];
          if (!node) return;
        ...
        async processMatches(grid) {
          ...
          while (cascades < MAX_CASCADES) {
            ...
            this.renderGrid(currentGrid); // 等待 150ms 观看消除效果

            await this.delay(150); // 重力下落 + 顶部补充新棋子

            currentGrid = this.engine.dropAndFill(currentGrid);
            this.renderGrid(currentGrid); // 等待 150ms 观看下落效果
